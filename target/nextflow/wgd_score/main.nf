    ...
    "resources" : [
      {
        ...
        "path" : "script.R",
        "is_executable" : true,
        "parent" : "file:/home/rcannood/workspace/cmgg/cmgg-structural/src/wgd_score/config.vsh.yaml"
      },
      {
        "type" : "file",
        "path" : "helper.R",
...
par <- list(
  "matrix" = $( if [ ! -z ${VIASH_PAR_MATRIX+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_MATRIX" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "scoring_mask" = $( if [ ! -z ${VIASH_PAR_SCORING_MASK+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_SCORING_MASK" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "scores" = $( if [ ! -z ${VIASH_PAR_SCORES+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_SCORES" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "plot" = $( if [ ! -z ${VIASH_PAR_PLOT+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_PLOT" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "gzip" = $( if [ ! -z ${VIASH_PAR_GZIP+x} ]; then echo -n "as.logical(toupper('"; echo -n "$VIASH_PAR_GZIP" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'))"; else echo NULL; fi ),
  "outliers" = $( if [ ! -z ${VIASH_PAR_OUTLIERS+x} ]; then echo -n "as.logical(toupper('"; echo -n "$VIASH_PAR_OUTLIERS" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'))"; else echo NULL; fi )
)
meta <- list(
